...
export var paperdoll_scene : PackedScene
...
func _ready():
	...
	remove_temporary_sprites()
	

	yield(get_tree().create_timer(0.75), "timeout") # give the city time to get ready
	if not is_connected("loot_ready", map_scene, "_on_loot_ready"):
		init(Global.current_city_map, null)


func init(mapScene, pathFollowObj):
	#gun = find_node("NPCGun")
	map_scene = mapScene
...
func get_random_paper_doll_name():
	# no longer needed. The Boss Spawner will provide the paperdoll and a weapon
	printerr("deprecated function get_random_paper_doll_name in BossNPC.gd")
	return
...
func remove_temporary_sprites():
	if has_node("Sprite/vizSpriteHiddenInCode"):
		$Sprite/vizSpriteHiddenInCode.queue_free()


func spawn_sprite(spriteName):
	# no longer needed. The Boss Spawner will provide the paperdoll and a weapon
	printerr("deprecated function spawn_sprite() in BossNPC.gd")
	return
